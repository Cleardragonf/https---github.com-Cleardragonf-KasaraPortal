...
For Each objTransFolder In objFSO.GetFolder(TransFolderPath).SubFolders
  ...
  Wscript.Echo objTransFolder.Name 
  If Mid (objTransFolder.Name, 1, 3) = "EDI" Then
     strTrans = objTransFolder.Name
     WScript.Echo "Number of Subfolders for " & objTransFolder.Name &": " & objTransFolder.Subfolders.Count
     WScript.Echo  "Currently Checking: " & TransFolderPath & "\" & strTrans & "\Archive\" & strYearFolder & "\" & strMonthFolder
	 ...
	 strFileOutput = ReportFolderPath & "\" & strYearFolder & "\" & strMonthFolder & "\" & "837D_DAILY_COUNT_RPT.txt"	
	...
	Wscript.Echo  "Current Zip folder working in : " & objZipFolder
  If intFolderYear = (Year(Date)) And intFolderMonth = (Month(Date)) Then
	...
	Else 
	   ...
	   Next
   End If
   Wscript.Echo "Number of files in zip Folder: " & ObjZipFolder.files.Count
   If objZipFolder.Files.Count <> 0 Then
...
'		 strSrchTrans = "CLM\*"
        ...
        If  Instr(UCase(objFile.Name), ".ZIP") = 0 And Instr(UCase(objFile.Name), "NCPDP") = 0 And objFile.Size > 0 Then 
            ...
            ' **** Logic to get date value of file that is part of file name
              ...
              Wscript.echo    "Currently Scanning - File Name: " & strSearchString 'Vidya
	        ...
	        'strSrchTransSet = "ST\" & strDelim & "270"
		 	...
		 	strSrchTransSet = "ST\" & strDelim & "837"
		 	strSrchTrans = "CLM\" & strDelim

  	       'If Mid(UCase(ObjFile.Name),14,4) = "270" And objFile.Size > 0 Then
  	         If Instr(UCase(objFile.Name), "837D") <> 0 And objFile.Size > 0 Then
		  ...
		  End If
                  'WScript.echo Mid(strData, 3, 2)
						...
						'wscript.echo Mid(strData, 1, 4) & intFolderMonth
